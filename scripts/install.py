...
def install():
    ...
    install_name = "destroyextension@arongranberg"
    ...
    if which("npm") is None:
        printc(RED, "npm could not be found. You need to install npm to build destroyextension. See https://www.npmjs.com/get-npm")
        exit(1)

    if which("node") is None:
        printc(RED, "node could not be found. You need to install node to build destroyextension. See https://nodejs.org/en/")
        exit(1)
    ...
    # Check if destroyextension is already installed
    ...
    if os.path.exists(install_path) or os.path.islink(install_path):
        if os.path.realpath(install_path) == os.path.realpath(dist_dir):
            ...
            printc(RED, f"You have installed destroyextension directly in the extensions folder. This is not supported anymore.")
            response = input(f"{RED}Would you like to move the installation to your home folder? [y/N]{RESET}")
            if response in ["y", "yes"]:
                new_path = os.path.expanduser("~/destroyextension")
            ...
            # There's already an actual folder (not a symlink) with a destroyextension install.
            # We need to delete that folder if we want to be able to install this version of the extension.
            printc(RED, f"destroyextension has already been installed separately, do you want to delete that installation?")
    ...
    if window_manager == "Type=x11":
        ...
        printc(YELLOW, "Installation succeeded. To activate destroyextension, please log out and log in again.")
